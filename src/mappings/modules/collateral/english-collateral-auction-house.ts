...
import { EnglishAuctionConfiguration, EnglishAuctionBid, CollateralAuction, getOrCreateCollateral } from '../../../entities'
import { fromWad, fromRay, fromRad } from '../../../utils/decimal'
import { dataSource } from '@graphprotocol/graph-ts'

export function handleModifyParametersAddress(event: ModifyParametersAddress): void {
  let what = event.params.parameter.toString()
  let collateral = getOrCreateCollateral(EnglishCollateralAuctionHouse.bind(dataSource.address()).collateralType(), event)
...
export function handleModifyParametersUint(event: ModifyParametersUint): void {
  ...
  if (what == 'bidIncrease') {
    config.bidIncrease = fromWad(val)
    ...
    config.bidToMarketPriceRatio = fromRay(val)
  }
...
export function handleDecreaseSoldAmount(event: DecreaseSoldAmount): void {
    ...
    bid.amountToSell = fromRad(event.params.amountToBuy)
...
export function handleIncreaseBidSize(event: IncreaseBidSize): void {
    ...
    bid.auction = auctionId
    bid.bidAmount = fromRad(event.params.rad)
